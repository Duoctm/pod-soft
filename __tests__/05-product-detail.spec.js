...
import { BASE_URL, PRODUCTS_URL } from "./utils/constants.js";
const PRODUCT_SLUG = "bella-3001";

// Helper: get exact button by visible name
const getButtonByText = (page, text) => page.getByRole("button", { name: text, exact: true });
...
test.describe("Product Detail Page Tests", () => {
	...
	test("PD003 - Verify Color options are displayed and selectable", async () => {
		...
		for (let i = 0; i < count / 10; i++) {
			...
			try {
				...
				console.warn(`Color button ${i + 1} not clickable or visible - skipped`);
			}
		}
	});
	...
	test("PD004 - Verify Size options are displayed and selectable", async () => {
		...
		const sizeButtons = pd.page.locator('div.flex-wrap button[title]:not([class*="rounded-full"])');
	...
	test("PD009 - Verify Error message for invalid quantity", async ({ page }) => {
		...
		const systemErrorAlert = page.locator('text="Something went wrong. Please try again later"');
	...
	test("PD016 - Verify Product Description is displayed and formatted correctly", async () => {
		...
		await expect(descriptionHeading).toBeVisible({ timeout: 15000 });
