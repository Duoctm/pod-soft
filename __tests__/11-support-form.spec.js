...
async function verifyDialog(page, expectedMessage) {
	...
	try {
		...
		const dialogButton = await page.locator('div[role="dialog"] button').first();
...
test.describe("Support Form Tests", () => {
	test.beforeEach(async ({ page }) => {
		try {
			await page.goto(SUPPORT_URL, { timeout: 15000 }); // Lower timeout to fail fast
	...
	//   await expect(firstNameInput).toBeFocused({ timeout: 15000 });
	...
	test("SP005 - Submit form with all valid data", async ({ page }) => {
		...
		await expect(page.locator("text=Support request created successfully")).toBeVisible({ timeout: 15000 });
		...
		try {
			...
			const dialogByText = await page.getByText("Your support request has been submitted successfully");
	...
	test("SP006 - Submit form with only mandatory fields filled", async ({ page }) => {
		const uniqueEmail = generateUniqueEmail("mailinator.com");
		const form = await page.locator("form.w-full.max-w-2xl");
	...
	test("SP008 - Submit with First Name empty (assuming mandatory)", async ({ page }) => {
		const uniqueEmail = generateUniqueEmail("mailinator.com");
		const form = await page.locator("form.w-full.max-w-2xl");
	...
	test("SP009 - Submit with Last Name empty (assuming mandatory)", async ({ page }) => {
		const uniqueEmail = generateUniqueEmail("mailinator.com");
		const form = await page.locator("form.w-full.max-w-2xl");
	...
	test("SP010 - Submit with Details empty (assuming mandatory)", async ({ page }) => {
		test.setTimeout(15000);
		const form = await page.locator("form.w-full.max-w-2xl");
	...
	test("SP011 - Submit with Email empty (assuming mandatory)", async ({ page }) => {
		const form = await page.locator("form.w-full.max-w-2xl");
	...
	test("SP012 - First Name - Verify maximum length boundary", async ({ page }) => {
		test.setTimeout(15000);
		const form = await page.locator("form.w-full.max-w-2xl");
	...
	test("SP013 - First Name - Verify exceeding maximum length", async ({ page }) => {
		test.setTimeout(15000);
	...
	test("SP014 - Last Name - Verify maximum length boundary", async ({ page }) => {
		test.setTimeout(15000);
		const form = await page.locator("form.w-full.max-w-2xl");
	...
	test("SP015 - Last Name - Verify exceeding maximum length", async ({ page }) => {
		const form = await page.locator("form.w-full.max-w-2xl");
	...
	test("SP018 - Email format - Spaces are trimmed and accepted", async ({ page }) => {
		const form = await page.locator("form.w-full.max-w-2xl");
	...
	test("SP019 - Invalid email format - Invalid characters in domain", async ({ page }) => {
		const form = await page.locator("form.w-full.max-w-2xl");
	...
	test("SP023 - Phone number with incorrect length (too short/long)", async ({ page }) => {
		const form = await page.locator("form.w-full.max-w-2xl");
	...
	test("SP024 - Company field - Accept alphanumeric & common symbols", async ({ page }) => {
		const form = await page.locator("form.w-full.max-w-2xl");
