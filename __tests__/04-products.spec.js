	...
	const testCases = [
		...
		},
		{
			...
			run: async (page) => {
				...
				const beforeTransform = await homeLink.evaluate((el) => getComputedStyle(el).transform);
				await homeLink.hover();
				await page.waitForTimeout(300);
				const afterTransform = await homeLink.evaluate((el) => getComputedStyle(el).transform);
		...
		},
		{
			...
			run: async (page) => {
				await page.goto(BASE_URL + "/products");
				const breadcrumbItems = await page.locator("nav a, nav span").allTextContents();
...

		{
			...
			inputData: "Open /products and check product name 'BELLA + CANVAS' and price format",
			run: async (page) => {
				await page.goto(BASE_URL + "/products");
				...
				const priceLocator = page.locator('[data-testid="ProductElement_PriceRange"]').first();
		...
		},
		{
			...
			run: async (page) => {
				...
				const productLink = page.locator('[data-testid="ProductElement"] a').first();
				...
				expect(actualText.trim().substring(0, 5)).toBe(expectedName.substring(0, 5));
			},
		},
		{
			id: "PR013",
			...
			run: async (page) => {
				...
				const img = page.locator('[data-testid="ProductElement"]').first().locator("img");
		...
		},
		{
			...
			run: async (page) => {
				await page.goto(BASE_URL + "/products");
				const card = page.locator('[data-testid="ProductElement"] a div div div h3', {
					hasText: "BELLA + CANVAS",
				});
		...
		// Passed
		{
			...
			run: async (page) => {
				...
				const unrelated = page.locator('[data-testid="ProductElement"]').first();
...

		{
			...
			inputData: "Press Tab 10 times and check if a product card receives focus",
			run: async (page) => {
				await page.goto(BASE_URL + "/products");
				...
				for (let i = 0; i < 10; i++) {
					...
					const focusedTestId = await page.evaluate(() => {
						...
						return parentWithTestId ? parentWithTestId.getAttribute("data-testid") : null;
					});
...

		{
			...
			run: async (page) => {
				...
				const gridsCount = await page.locator('[data-testid="ProductList"]').count();
				...
				const getGridColumns = async () => {
					const grid = page.locator('[data-testid="ProductList"]');
					await expect(grid).toBeVisible({ timeout: 10000 });
					return await grid.evaluate((el) => getComputedStyle(el).gridTemplateColumns);
				};
		...
		{
			id: "PR027",
			description: "'No products found' message appears when search yields no results",
			inputData: "Fill search with 'somethingthatdoesnotexist' and click submit",
			run: async (page) => {
				await page.goto(BASE_URL + "/products");
