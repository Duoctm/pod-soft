import { SUPPORT_URL } from "./utils/constants.js";
import { test, expect } from "@playwright/test"; // Changed from require
import { generateUniqueEmail } from "./utils/testDataHelper.js"; // Changed from require
...
async function verifyDialog(page, expectedMessage) {
	...
	try {
		...
		const buttonByText = page.getByText(/ok/i);
		...
		const dialogButton = page.locator('div[role="dialog"] button').first();
...
test.describe("Support Form Tests", () => {
	...
	test("SP001 - Check alignment of input fields", async ({ page }) => {
		const form = page.locator("form.w-full.max-w-2xl");
		...
		for (let i = 0; i < fieldPositions.length; i++) {
			for (let j = i + 1; j < fieldPositions.length; j++) {
				...
				const horizontalOverlap = Math.max(
					0,
					Math.min(field1.box.x + field1.box.width, field2.box.x + field2.box.width) -
						Math.max(field1.box.x, field2.box.x),
				);

				const verticalOverlap = Math.max(
					0,
					Math.min(field1.box.y + field1.box.height, field2.box.y + field2.box.height) -
	...
	test('SP002 - Check "Send" button is disabled until mandatory fields are filled', async ({ page }) => {
	...
	test("SP003 - Check responsive layout", async ({ page }) => {
		...
		await page.screenshot({ path: "./test-results/images/mobile-layout.png", fullPage: true });

		// Test desktop viewport
		await page.setViewportSize({ width: 1280, height: 800 });
		await expect(form).toBeVisible();
		await page.waitForTimeout(1000);
		await page.screenshot({ path: "./test-results/images/desktop-layout.png", fullPage: true });
	});
	...
	test("SP007 - Submit form with all valid data", async ({ page }) => {
		...
		await expect(page.locator("text=Support request created successfully")).toBeVisible({ timeout: 10000 });

		await page.pause();

		console.log("Current URL:", page.url());

		await page.screenshot({ path: "./test-results/images/before-dialog.png" });
		...
		try {
			...
			const dialogByText = page.getByText("Your support request has been submitted successfully");
	...
	test("SP008 - Submit form with only mandatory fields filled", async ({ page }) => {
		const uniqueEmail = generateUniqueEmail("mailinator.com");
		const form = page.locator("form.w-full.max-w-2xl");
	...
	test("SP010 - Submit with First Name empty (assuming mandatory)", async ({ page }) => {
		const uniqueEmail = generateUniqueEmail("mailinator.com");
		const form = page.locator("form.w-full.max-w-2xl");
	...
	test("SP011 - Submit with Last Name empty (assuming mandatory)", async ({ page }) => {
		const uniqueEmail = generateUniqueEmail("mailinator.com");
		const form = page.locator("form.w-full.max-w-2xl");
	...
	test("SP012 - Submit with Details empty (assuming mandatory)", async ({ page }) => {
		test.setTimeout(120000);
		const form = page.locator("form.w-full.max-w-2xl");
		...
		//Wait for submitting message to disappear
		await expect(page.locator("text=Support request created successfully")).toBeVisible({ timeout: 30000 });
	...
	test("SP013 - Submit with Email empty (assuming mandatory)", async ({ page }) => {
		const form = page.locator("form.w-full.max-w-2xl");
	...
	test("SP016 - First Name - Verify maximum length boundary", async ({ page }) => {
	...
	test("SP017 - First Name - Verify exceeding maximum length", async ({ page }) => {
		...
		try {
			...
			const dialogByText = page.getByText(
		...
		await expect(form.locator('input[name="firstName"]')).toHaveValue(tooLongName);
		await expect(form.locator('input[name="lastName"]')).toHaveValue("Doe");
		await expect(form.locator('input[name="email"]')).toHaveValue("john@example.com");
		await expect(form.locator('input[name="phoneNumber"]')).toHaveValue("1234567890");
		await expect(form.locator('input[name="company"]')).toHaveValue("ABC Corp");
		await expect(form.locator('input[name="address"]')).toHaveValue("123 Street");
	...
	test("SP019 - Last Name - Verify maximum length boundary", async ({ page }) => {
		test.setTimeout(120000);
		const form = page.locator("form.w-full.max-w-2xl");
	...
	test("SP020 - Last Name - Verify exceeding maximum length", async ({ page }) => {
		// Enable debug logging
		page.on("console", (msg) => console.log(msg.text()));
		...
		await expect(form.locator('input[name="lastName"]')).toHaveValue(tooLongName);
		await expect(form.locator('input[name="email"]')).toHaveValue("john@example.com");
		await expect(form.locator('input[name="phoneNumber"]')).toHaveValue("1234567890");
		await expect(form.locator('input[name="company"]')).toHaveValue("ABC Corp");
		await expect(form.locator('input[name="address"]')).toHaveValue("123 Street");
	...
	test("SP023 - Email format - Spaces are trimmed and accepted", async ({ page }) => {
		const form = page.locator("form.w-full.max-w-2xl");
	...
	test("SP024 - Invalid email format - Invalid characters in domain", async ({ page }) => {
		const form = page.locator("form.w-full.max-w-2xl");
		...
		// Verify form retains the entered data
		await expect(form.locator('input[name="email"]')).toHaveValue("test@exa!mple.com");
	});
	...
	test("SP026 - Phone number with letters", async ({ page }) => {
		...
		// Verify form retains the entered data
		await expect(form.locator('input[name="phoneNumber"]')).toHaveValue("abc123456");
	});
	...
	test("SP027 - Phone number with special characters", async ({ page }) => {
		...
		// Verify form retains the entered data
		await expect(form.locator('input[name="phoneNumber"]')).toHaveValue("123456%780");
	});

	test("SP028 - Phone number with incorrect length (too short/long)", async ({ page }) => {
		const form = page.locator("form.w-full.max-w-2xl");
	...
	test("SP030 - Company field - Accept alphanumeric & common symbols", async ({ page }) => {
		const form = page.locator("form.w-full.max-w-2xl");
	...
	test("SP032 - HTML/JS injection attempt", async ({ page }) => {
		...
		await form.locator('input[name="firstName"]').fill(injectionData.xss.firstName);
		await form.locator('input[name="lastName"]').fill(injectionData.xss.lastName);
		await form.locator('input[name="email"]').fill(injectionData.xss.email);
		await form.locator('input[name="phoneNumber"]').fill(injectionData.xss.phoneNumber);
		...
		await form.locator('textarea[name="details"]').fill(injectionData.xss.details);
	...
	test("SP033 - SQL injection attempt in various fields", async ({ page }) => {
		...
		await expect(page.locator("text=/SQL|database|error/i")).toBeHidden();
	});
});
