...
class CartPage {
	...
	async getTotalItemPrices() {
		...
		const quantityInputs = await this.page
			.locator('input[type="number"]')
			.evaluateAll((inputs) => inputs.map((input) => parseInt(input.value, 10)));
	...
	async getItemPrice(index = 0) {
		const priceText = await this.getItem(index).locator("p:has-text('$')").innerText();
