	...
	const loginTestCases = [
		{
			...
			expected: "User should be logged in successfully and redirected to the home page",
			shouldPass: true,
		},
		{
			id: "LG002",
		...
		},
		{
			...
			shouldPass: false,
			expectedError: "Password must be at least 8 characters long and contain at least one lowercase letter",
		},
		{
			id: "LG007",
		...
		},
		{
			...
			expected: "Error messages for both fields: 'Email is required', 'Password is required'",
			shouldPass: false,
			expectedError: "Email is required",
		},
		{
		...
		test(`${testCase.id} - ${testCase.description}`, async ({ page }) => {
			...
			try {
				...
				if (testCase.shouldPass) {
					...
					await expect(page).locator("h1", { hasText: "Welcome to ZoomPrints" }).toBeVisible();
