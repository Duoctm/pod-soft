...
test.describe("Add product to cart", () => {
	...
	test("TC019 - Login and add product to cart", async ({ page }) => {
		...
		try {
			...
			const option = page.locator('[role="option"][aria-disabled="false"]').first();
			if (await option.isVisible()) {
				await option.click();
			}

			// 5. Kiểm tra input số lượng
			const quantityInput = page.locator('div:has-text("Qty:") input[type="number"]');
			...
			const confirmationToast = page.locator('.toast-message, [role="alert"], text=Added to cart');
