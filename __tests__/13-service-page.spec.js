...
const services = [
	...
	},
	{
		name: "Hard Goods",
		imgAlt: "Hard Goods",
		descriptions: [/Premium drinkware options/i, /Mimaki and Grando machines/i, /Professional finishing/i],
	},
	{
		name: "Custom Boxes",
		imgAlt: "Custom Boxes",
		descriptions: [/Eye-catching designs/i, /Perfect for executive kits/i, /Premium packaging solutions/i],
	},
	{
		name: "Canvas Print",
		imgAlt: "Canvas Print",
		descriptions: [/Latex HP printers/i, /Vibrant colors/i, /Durable materials/i],
	},
];
...
test.describe("Service Page Test Suite", () => {
	...
	test("SV013 - Test Service link with JavaScript disabled", async ({ browser }) => {
		...
		try {
			...
			console.warn("⚠️ Warning: Page may not support no-JS users or timed out.", err.message);
	...
	test("SV018 - Test image hover scale effect", async ({ page }) => {
		for (const service of services) {
			const heading = page.locator("h2", { hasText: service.name }).first();
			const wrapper = heading.locator("xpath=ancestor::*[contains(@class, 'group')]").first();
