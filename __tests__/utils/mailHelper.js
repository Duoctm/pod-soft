// utils/mailHelper.js
import { MAILINATOR_BASE_URL } from "./constants";
...
async function getVerificationLinkFromMailinator(page, emailAddress) {
	...
	const firstEmailSubjectCellSelector = '//table[contains(@class, "table")]//tbody//tr[1]/td[3]';
	try {
		...
		console.error("Timeout waiting for email list in Mailinator. Saving screenshot.");
		await page.screenshot({
			path: `mailinator_inbox_timeout_${inboxName}.png`,
			fullPage: true,
		});
		throw new Error(`Timeout waiting for email list in Mailinator for ${inboxName}.`);
	}
	...
	if (!clickedEmail) {
		...
		try {
			await page.locator(firstEmailSubjectCellSelector).first().click({ timeout: 20000 });
		} catch (e) {
			...
			throw new Error(`Failed to click any email in Mailinator inbox for ${inboxName}.`);
		}
	}

	const iframeEmailBody = page.frameLocator("#html_msg_body");
	if (!iframeEmailBody) {
		console.error("Could not find Mailinator email body iframe. Saving screenshot.");
		await page.screenshot({
	...
	try {
		await verificationLinkLocator.first().waitFor({ state: "visible", timeout: 45000 });
		console.log("Verification link locator found in iframe.");
	} catch (e) {
		console.error("Timeout waiting for verification link in Mailinator email body. Saving screenshot.");
		try {
		...
		throw new Error("Verification link not found or not visible in Mailinator email body.");
	}

	const linkHref = await verificationLinkLocator.first().getAttribute("href");
	if (!linkHref) {
		console.error("Verification link href attribute is null. Saving screenshot.");
		await page.screenshot({
...
export { getVerificationLinkFromMailinator };
