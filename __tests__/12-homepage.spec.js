...
test.describe("ZoomPrints UI Test Cases", () => {
	...
	test("HP002 - Search with valid product name", async ({ page }) => {
		...
		await expect(page.locator("text=Search results for")).toContainText(`Search results for "${keyword}"`);
	});
	...
	test("HP003 - Login button redirects to login page", async ({ page }) => {
		...
		await expect(page).toHaveURL(/https:\/\/accounts\.mypodsoftware\.io\.vn\/realms\/keycloak/);
	});
	...
	test("HP006 - Navigation links restricted when logged out", async ({ page }) => {
		await page.click("div.flex.items-center.justify-center.rounded-md.p-2");

		await expect(page).toHaveURL(/https:\/\/accounts\.mypodsoftware\.io\.vn\/realms\/keycloak/);
	});

	test("HP007 - Search with invalid product returns no results", async ({ page }) => {
	...
	test("HP008 - Login button disabled when already logged in (mock)", async ({ page }) => {
	...
	test('HP009 - "Welcome to ZoomPrints" is readable on all screen sizes', async ({ page }) => {
	...
	test("HP010 - Service features are displayed correctly", async ({ page }) => {
		const features = ["Best Quality", "Secure Payment", "Professional", "Competitive Pricing"];
		for (const text of features) {
			await expect(page.locator(`text=${text}`)).toBeVisible();
		}
	});

	test("HP011 - Resize to small and check text readability", async ({ page }) => {
		await page.setViewportSize({ width: 320, height: 480 });
		const allText = await page.locator("body").textContent();
		expect(allText?.length).toBeGreaterThan(0);
	});

	test("HP012 - Page with JS disabled (manual/headless mode test)", async ({ browser }) => {
	...
	test('HP014 - "Learn More" button is visible and clickable', async ({ page }) => {
	...
	test("HP015 - Each service card shows correct image, title, and description", async ({ page }) => {
	...
	test('HP016 - "Learn More" on each card links correctly', async ({ page }) => {
