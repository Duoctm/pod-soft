...
import { SUPPORT_URL } from "../../../__tests__/utils/constants.js";
import * as testDataHelper from "../../../__tests__/utils/testDataHelper.js";
...
Then("I should see {string} message", { timeout: 60000 }, async function (message) {
	...
	try {
		...
		await this.page.screenshot({ path: `test-results/cucumber/images/error-${Date.now()}.png` });
...
Then("the form should be submitted successfully", { timeout: 60000 }, async function () {
	await expect(this.page.locator("text=Support request created successfully")).toBeVisible({
		timeout: 30000,
	});
});
...
Then("I should see an error message about field length", async function () {
	await expect(
		this.page.locator("text=Error creating support request: value too long for type character varying(255)"),
...
Then("the form should be properly displayed on mobile", async function () {
	...
	await this.page.screenshot({ path: "test-results/cucumber/images/mobile-layout.png", fullPage: true });
});
...
Then("the form should be properly displayed on desktop", async function () {
	const form = await this.page.locator("form.w-full.max-w-2xl");
	await expect(form).toBeVisible({ timeout: 30000 });
	await this.page.screenshot({ path: "test-results/cucumber/images/desktop-layout.png", fullPage: true });
});
