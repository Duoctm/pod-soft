				...
				<div className="relative flex flex-1 flex-col justify-between">
					...
					<div className="flex flex-col items-start justify-start gap-y-2 md:flex-row md:items-center mt-2">
						...
						<div className="hidden items-center justify-center gap-2 md:flex">
							{Array.isArray(item.metadata) && item.metadata.length > 0 && (item.metadata?.find((item) => item.key === "design")?.value != 'null') && (
			...
			{!checkout || !items || items.length < 1 ? (
				!loading ? (
					<section className="mx-auto max-w-7xl py-2">
						...
						<LinkWithChannel
							href="/products"
							className="inline-block max-w-full rounded border border-transparent bg-[#F58A71] px-6 py-2 text-center font-medium text-[#FFFFFF] hover:bg-[#F58A71] aria-disabled:cursor-not-allowed aria-disabled:bg-[#C59CAE] sm:px-16"
						>
							Explore products
						</LinkWithChannel>
					</section>
				...
				<form className="mt-2 flex w-full flex-1 flex-col-reverse gap-x-4 gap-y-4 md:flex-row ">
					...
					>
						...
						<div className="mt-2 flex flex-1 justify-end py-2">
							<p className="flex items-center gap-x-2 text-neutral-900">
								{`Subtotal ( ${formatNumber(totalSubtotal)} items ) : `}
					...
					<div className="h-full w-full rounded border bg-neutral-50 p-6   px-4 py-2  md:max-w-[350px] lg:sticky lg:top-40 ">
						<div className="">
							<div className="flex items-center flex-wrap justify-between gap-2 py-2">
								<div>
									<p className="font-medium text-neutral-900">{`Subtotal ( ${formatNumber(totalSubtotal)} items ) : `}</p>
								</div>
