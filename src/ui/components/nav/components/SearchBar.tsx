...
import { OrderDirection, type ProductListItemFragment, ProductOrderField } from "@/gql/graphql";
...
export const SearchBar = ({ channel }: { channel: string }) => {
	...
	async function onSubmit(formData: FormData) {
		void redirectSearchPage(formData, channel);
		resetToDefault();
	}

	const handleProductNagivationToSearchPage = (channel: string, debouncedValue: string) => {
		router.push(`/${channel}/search?query=${debouncedValue}`);
		router.refresh();
		resetToDefault();
	};
		...
		>
			<label className="w-full">
				...
				<input
					...
					autoComplete="on"
					required
					className="h-12 w-full rounded-lg border border-[#8B3958] bg-white px-4 py-2 pr-12 text-sm text-black placeholder:text-neutral-500 focus:border-[#8B3958] focus:outline-none focus:ring-2 focus:ring-[#8B3958] focus:ring-opacity-50"
				/>
			</label>
			...
			<div className="absolute left-0 right-0 top-full z-50 max-h-[300px] overflow-y-auto rounded-b-lg border-[#8B3958] bg-white shadow-lg">
				{isLoading && debouncedValue ? (
					<div className="flex items-center justify-center p-4">
						<div className="h-6 w-6 animate-spin rounded-full border-2 border-[#8B3958]"></div>
					</div>
				) : (
					<ul className="divide-y divide-gray-100 rounded-md">
						{searchResults.length > 0 ? (
							searchResults.map((product) => (
								<li key={product.id} className="hover:bg-gray-50 border-2 border-[#8B3958] border-t-transparent rounded-b-md">
									<div
										onClick={() => handleProductNagivationToSearchPage(channel, debouncedValue)}
										className="flex items-center gap-4 p-4"
									>
										<div className="relative h-10 w-10 flex-shrink-0 overflow-hidden rounded-md">
											<Image src={product.thumbnail?.url as string} alt={product.name} fill />
										</div>
