...
const textStyles = {
  base: "text-white text-[14px] md:text-[16px] font-[400] leading-[24px] md:leading-[26px] flex-1",
...
const renderNavigationColumn = ({ links }: typeof footerNavigation[0], channel: string, title: string) => (
  <div className="flex flex-col gap-[16px] md:gap-[24px] flex-1 w-full">
    <div className={textStyles.heading}>{title}</div>
    <div className="flex flex-row gap-[8px] md:gap-[10px] w-full flex-1">
...
export async function Footer({ channel }: FooterProps) {
  ...
  return (
    <div className="bg-[#1C1C1C] py-8 md:py-16 flex flex-col">
      <Wrapper className="flex flex-col  md:flex-row items-start flex-1 md:items-end justify-between gap-8 md:gap-0">
        ...
        <div className="flex flex-1 flex-row items-center gap-8 md:gap-16 w-full md:w-auto w-full justify-end">
          {footerNavigation.map((section, index) => (
            <div key={index} className="flex flex-1 justify-end">{renderNavigationColumn(section, channel, section.title)}</div>
          ))}
        </div>
      </Wrapper>
