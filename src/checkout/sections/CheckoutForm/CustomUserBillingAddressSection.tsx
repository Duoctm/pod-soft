...
const UserBillingAddressFormValidationSchema = Yup.object().shape({
...
const BillingFormikStateSyncer: React.FC<{
	onBillingAddressChange: (values: AddressFormData | null, isValid: boolean) => void; // Allow null values
	...
	useEffect(() => {
		...
		debounceTimeoutRef.current = setTimeout(() => {
			// Call the original callback after a delay (e.g., 300ms)
			onBillingAddressChange(values, isValid);
		}, 3000);
	...
	}, [values, isValid, onBillingAddressChange]);
...
export const BillingAddressForm: React.FC<BillingAddressFormProps> = ({
	...
	onBillingAddressChange,
	countriesData,
	apiErrors, // Destructure the prop
}) => {
	...
	return (
		<Formik<AddressFormData>
			...
			validationSchema={UserBillingAddressFormValidationSchema}
			onSubmit={() => {}}
			enableReinitialize
			validateOnMount
		>
			{({ errors, touched }) => (
				<Form className="space-y-4 rounded-lg bg-white font-sans">
					{/* Syncer remains the same */}
					<BillingFormikStateSyncer onBillingAddressChange={onBillingAddressChange} />

					<h2 className="mb-6 text-xl font-semibold text-gray-800">Billing Address</h2>

					{/* Country Field */}
					<div>
						<label htmlFor="billing-country" className="mb-1 block text-sm font-medium text-gray-700">
							Country *
						</label>
						<Field
							as="select"
							...
							className={`w-full border px-3 py-2 ${
								(errors.country && touched.country) || apiErrors?.country
									? "border-red-500"
									: "border-gray-300"
						...
						{!errors.country && apiErrors?.country && (
					...
					{/* First Name Field */}
					<div>
						<label htmlFor="billing-firstName" className="mb-1 block text-sm font-medium text-gray-700">
							First name *
						</label>
						<Field
							type="text"
					...
					{/* Last Name Field */}
					<div>
						<label htmlFor="billing-lastName" className="mb-1 block text-sm font-medium text-gray-700">
							Last name *
						</label>
						<Field
							type="text"
					...
					{/* Company Field */}
					<div>
						<label htmlFor="billing-company" className="mb-1 block text-sm font-medium text-gray-700">
							Company
						</label>
						<Field
							type="text"
							...
							className={`w-full rounded-md border px-3 py-2 shadow-sm focus:border-indigo-500 focus:outline-none focus:ring-indigo-500 sm:text-sm ${
								apiErrors?.company ? "border-red-500" : "border-gray-300"
							}`}
						/>
					...
					<div>
						<label htmlFor="billing-streetAddress1" className="mb-1 block text-sm font-medium text-gray-700">
							Street address *
						</label>
						<Field
							type="text"
					...
					<div>
						<label htmlFor="billing-streetAddress2" className="mb-1 block text-sm font-medium text-gray-700">
					...
					{/* City Field */}
					<div>
						<label htmlFor="billing-city" className="mb-1 block text-sm font-medium text-gray-700">
							City *
						</label>
						<Field
							type="text"
					...
					{/* Zip Code Field */}
					<div>
						<label htmlFor="billing-zipCode" className="mb-1 block text-sm font-medium text-gray-700">
							Zip code *
						</label>
						<Field
							type="text"
					...
					{/* Phone Number Field */}
					<div>
						<label htmlFor="billing-phoneNumber" className="mb-1 block text-sm font-medium text-gray-700">
							Phone number
						</label>
						<Field
							type="tel"
							...
							name="phoneNumber"
							className={`w-full rounded-md border px-3 py-2 shadow-sm focus:border-indigo-500 focus:outline-none focus:ring-indigo-500 sm:text-sm ${
								apiErrors?.phoneNumber ? "border-red-500" : "border-gray-300"
							}`}
						/>
						<ErrorMessage name="phoneNumber">
							{(msg) => <div className="mt-1 text-sm text-red-600">{msg}</div>}
						</ErrorMessage>
						{apiErrors?.phoneNumber && !errors.phoneNumber && (
